...
### Version 2026.5 (Implemented) — Animator Bottom-Left Origin + Calendar Versioning
- **Sketch frame origin at bottom-left** — the Animator's `Size(w,h)` frame is now anchored so `(0,0)` is the frame's bottom-left corner (previously the centre). Sketch coordinates run `x ∈ [0,w]`, `y ∈ [0,h]`, Y-up; `AnimCanvas.SetBoundary` (plus the resize handler and boundary-outline draw) use `0..w × 0..h` bounds. The world↔screen transform was already general — only the frame bounds changed. **Code2Viz's own canvas is unchanged (still centre-origin)** — the two apps differ here by design.
- **Origin axis lines removed** — `AnimCanvas` no longer draws the X/Y lines through the origin (the background grid was already disabled); only the faint frame outline remains.
- **Default boilerplate updated** — `Templates.DefaultSketch` declares `width`/`height` fields and offsets its orbiting circle to the frame centre (`+ width/2`, `+ height/2`) so it stays visible under the new origin.
- **Calendar versioning (`YEAR.MONTH.PATCH`)** — releases are versioned by date: `scripts/release.ps1` stamps year/month from the release date and increments patch within a month (resetting to 0 on a new month or year), replacing the old `-Bump major/minor/patch`. `Directory.Build.props` + `installer.iss` remain the two synced version sources (`2.0.0` → `2026.5.0`).
