...
32. **`PolygonClipper` (`C2VGeometry/Operations/PolygonClipper.cs`) delegates polygon booleans to the Clipper2 library** (`Clipper2` NuGet package, namespace `Clipper2Lib`), added to `C2VGeometry.csproj` and inherited transitively by every consumer (Code2Viz, Animator, Tests, McpServer). It replaced a hand-rolled Greiner-Hormann tracer that failed on degenerate inputs: a polygon **vertex landing exactly on another polygon's edge** (the reported bug — a circle centered on a rectangle's corner samples vertices onto the rect edges, so G-H found *zero* crossings and wrongly returned the regions as disjoint → `Union` gave `null`), and **full collinear shared-edge bands** (mis-unioned to one input). The Clipper2 path uses `ClipperD(precision: 6)` (double API, scales internally) + `FillRule.NonZero`, executes into a `PolyTreeD`, then `CollectOuterContours` (flat `Union/Intersect/Difference` — non-hole contours only, islands included) or `BuildPolygonsWithHoles` (`*WithHoles` variants — outer + immediate hole children). `Xor(VPolygon,VPolygon)` stays `(A−B)+(B−A)` so it returns hole-free pieces (the historical flat contract). **`PointInPolygonTest` is unchanged** — still the local ray-cast, used widely outside booleans. **`Clipper2Lib.dll` must ship**: the Code2Viz installer enumerates DLLs explicitly, so `installer.iss` has an explicit `Clipper2Lib.dll` line next to `C2VGeometry.dll`; Animator's bundle is wildcarded so it's already covered. Guarded by `BooleanOpsTests` (collinear-band, donut-with-hole) and `RegionFromClosedCurveTests.Union_CircleCenteredOnRectangleCorner_MergesNonNull`.
...
35. **Animator's `AnimCanvas` supports cursor-anchored wheel zoom and double-click zoom-extents (`Animator/Canvas/AnimCanvas.cs`).** The old comment "zoom is intentionally disabled" is gone — `OnMouseWheel` calls `ZoomAt(cursor, factor)` (~10%/notch, clamped to `[1e-4, 1e6]`, solving pan so the world point under the cursor stays fixed), and a **left double-click** (`e.ClickCount == 2` in `OnMouseDown`, since `FrameworkElement` has no `MouseDoubleClick`) calls `ZoomExtents()` → fits the sketch boundary if one was declared via `Size()`, else the bounds of all visible shapes (skipping non-finite bounds like VRay/VXLine). The **sketch boundary outline** is toggleable: `AnimCanvas.ShowBoundary` gates only *rendering* (the boundary is still tracked for zoom-extents when hidden), wired to **View > Canvas Border** and persisted in `AnimatorSettings.CanvasBorderVisible` (note 23). Applied in `ApplyPersistedSettings`; saved in `CanvasBorderMenuItem_Click`.
