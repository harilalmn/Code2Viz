...
6. **VPoint.Internal(x, y)** creates points without auto-registration (for intermediate calculations). **VLine.Internal(start, end)** does the same for line segments — use it in any utility code that needs a `VLine` purely as a data container (segment tessellation, self-intersection scans, etc.) so the line does not pollute the canvas.
