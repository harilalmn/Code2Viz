    ...
    public class DocGenerator
    {
        ...
        private void InitializeSummaries()
        {
            _summaries = new Dictionary<string, string>
            {
                ...
                { "Animator", "Sub-project of Code2Viz: a standalone WPF app (Animator.exe) for p5.js-style sketches. Lives in the Animator/ folder; depends only on C2VGeometry.csproj. User code subclasses Animator.Sketching.Sketch and overrides Setup() (runs once) and Draw() (runs every frame). Geometry uses C2VGeometry types — shapes auto-register each frame and the canvas re-renders the current set. Switch between apps via the Switch to Animator / Switch to Project buttons. Both apps prompt to save unsaved edits before switching or closing." },
                { "Animator.Sketching.Sketch", "Base class for Animator sketches. Override Setup() for one-time init and Draw() for per-frame work. Protected helpers: Size(w,h) declares the logical drawing area (canvas auto-zooms to fit); Background(color) sets the canvas background; Loop()/NoLoop() resume/pause the frame loop. Framework-populated state: FrameCount, ElapsedSeconds, DeltaSeconds, Width, Height, MouseX, MouseY, MousePressed, KeyPressed, LastKey. Persistent state lives in fields on the subclass — locals reset each call." },
