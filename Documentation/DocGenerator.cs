    ...
    public class DocGenerator
    {
        ...
        private void InitializeSummaries()
        {
            _summaries = new Dictionary<string, string>
            {
                ...
                // Boolean Operations
                { "BooleanOps", "Static class providing polygon boolean operations using the robust Clipper2 library. Supports Union (combine polygons), Intersect (overlapping area), Difference (subtract), Xor (symmetric difference), OffsetPolygon (grow/shrink), OffsetPolygonSafe (safe inward offset), MaxSafeInwardOffset, MakeSimple (resolve self-intersections), HasSelfIntersections, Simplify (Douglas-Peucker algorithm), Area calculation, and PointInPolygon (ray casting). Also provides WithHoles variants (DifferenceWithHoles, IntersectWithHoles, UnionWithHoles) that return PolygonWithHoles objects." },
...
var fixedRange = new ChartOptions { YMin = 0, YMax = 100, YTickCount = 5 };" }
        ...
        private void InitializeMemberDescriptions()
        {
            _memberDescriptions = new Dictionary<string, string>
            {
                ...
                { "VLine.SetBounds", "Trims the line in place: the parameter sub-range [startParameter, endParameter] becomes the new [0, 1]. The Start/End point instances are preserved (X/Y are updated). Parameters are clamped to [0,1] and swapped if reversed." },
                ...
                { "VEllipse.SetBounds", "Trims the ellipse in place: the parameter sub-range [startParameter, endParameter] becomes the new [0, 1]. StartAngle/EndAngle are rescaled to span the new range. Parameters are clamped to [0,1] and swapped if reversed." },
                ...
                { "VBezier.SetBounds", "Trims the Bezier in place: the parameter sub-range [startParameter, endParameter] becomes the new [0, 1]. Uses De Casteljau subdivision twice (split at end, then at start/end) for an exact trim. P0..P3 instances are preserved (X/Y are updated). Parameters are clamped to [0,1] and swapped if reversed." },
